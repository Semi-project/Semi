			...
			<tbody>
				<c:forEach items="${boardList }" var="board">
					<tr>
						<td>${board.boardno }</td>
						<td><a
							href="/notice/view?boardno=${board.boardno }&curPage=${paging.curPage}">${board.title }</a></td>
						<td>${board.userid }</td>
						<td>${board.hit }</td>
		...
		<div id="pagingBox" class="text-center">
			<ul class="pagination pagination-sm">
				...
				<c:if test="${paging.curPage ne 1 }">
					<li><a href="/notice/list" aria-label="First"> <span
				...
				<c:if test="${paging.curPage ne 1 }">
					<li><a href="/notice/list?curPage=${paging.curPage-1 }"
