		...
		<div id="top" class="">
			<form method="GET" name="adoption" id="adoption"
				...
				<table class="board">
					<thead>
						<tr>
							<th style="width: 15%; text-align: center;">결제 종류</th>
							<th style="width: 20%; text-align: center;">동물번호&후원내역</th>
							<th style="width: 20%; text-align: center;">결제금액</th>
							<th style="width: 20%; text-align: center;">결제자</th>
							<th style="width: 30%; text-align: center;">결제날짜</th>
						</tr>
					</thead>
					...
					<tbody>
						...
						<c:forEach items="${charityList }" var="charity">
							<tr>
								...
								<td class="board_title" style="text-align: center;" >${charity.name }</td>
								<td>${charity.paid_amount }</td>
								<td>${charity.userid }</td>
								<td><fmt:formatDate value="${charity.charity_date }"
...
<div class="text-center">
	<ul class="pagination pagination-sm">
		...
		<c:if test="${paging.curPage ne 1 }">
			<li><a href="/mypage/charity?search=${paging.search }"
		...
		<c:if test="${paging.curPage ne 1 }">
			<li><a
				href="/mypage/charity?curPage=${paging.curPage-1 }&search=${paging.search }"
		...
		<c:forEach begin="${paging.startPage }" end="${paging.endPage }"
			...
			<c:if test="${paging.curPage eq i}">
				<li class="active"><a
					href="/mypage/charity?curPage=${i }&search=${paging.search }">${i }</a></li>
			</c:if>
