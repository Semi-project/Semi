...
public class Paging {
	...
	public Paging(int totalCount, int curPage, int listCount, int pageCount) {
		this.setTotalCount(totalCount);
		this.setCurPage(curPage); // 현재 페이지 
		this.setListCount(listCount); // 한 페이지당 출력될 게시글 수 (직접 설정함)
		this.setPageCount(pageCount);  // 한 화면에 출력될 페이지 수 (직접 설정함)
	...
	private void makePaging() {
		...
		// 기본값 설정
		if (curPage == 0)
			setCurPage(1); // 기본값으로 첫 페이지(1) 세팅
		if (pageCount == 0)
			setPageCount(10); // 한 화면에 보이는 페이지수 기본값(10) 세팅
		if (listCount == 0)
			setListCount(10); // 한 화면에 보이는 게시글수 기본값(10) 세팅
